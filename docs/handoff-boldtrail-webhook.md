...
- **JSON contract, not raw BoldTrail field names.** Since Zapier's Action step lets Tyler remap field names freely, this route defines the exact JSON keys it expects (`firstName`, `lastName`, `email`, `phone`, `isSeller`, `leadId`, `leadStatus`, `quality`, `hashtags`, `createdAt`, `sourceUrl`, `howAddedToDatabase`, `onDrip`, `leadDetailsLink`, `assignedAgentEmail`, `assignedAgentName`, `emailStatus`, `hasLender`, `leadCity`/`leadState`/`leadZip`, `street`, `sellerStreet`/`sellerCity`/`sellerState`/`sellerZip`/`sellerFullAddress`) — Tyler maps BoldTrail's trigger fields onto these exact keys when finishing the Zapier Action step, rather than this route guessing Zapier's internal naming.
